...
with edit_col3:
    save_position_button = st.button("Guardar posición", use_container_width=True)
...
if save_position_button and selected_player_to_edit:
    try:
        ...
        st.cache_data.clear()
        st.success(
            f"Posición guardada correctamente: {selected_player_to_edit} → {format_position_name(new_position_group)}. "
            "El cambio se aplicará al próximo recálculo métricas enriquecidas y clusters."
        )
